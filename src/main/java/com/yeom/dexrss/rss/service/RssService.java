...
public class RssService {
    ...
    public void findAndUpdateRssItem(String baseUrl, String category) {
        ...
        for (Element liElement : liElementList) {
            ...
            a.ifPresent(x -> items.add(RssItemDto.builder()
                    .title(title)
                    .link(x)
                    .page(link)
                    .category(category)
...
//        // Base URL + path 를 조회 해서 html 을 찾고 그 중 게시글 패턴에 맞는 항목 리스트 추출
//        Map<String, String> itemLinks = magnetLinkPollingService.findItemLinks(baseUrl, uriPath, itemSelector);
//        log.info("HTML 바디에서 찾은 아이템 링크 수: {}", itemLinks.size());
//
//
//        // 웹에서 조회된 URL 기준 루프
...
//            // 링크가 DB에 있는 값인지 비교
//            if(dbList.stream().anyMatch(x -> x.getLink().equalsIgnoreCase(link))){
//                return;
//            }
...
//            a.ifPresent(x -> items.add(RssItemDto.builder().title(key).link(x).page(link).category(category).build()));
//        });
        log.info("new items: {}", items);
        // DB 에 업데이트
        if (!items.isEmpty()) {
